...
def name_of(chat_id: int) -> str:
    return USERS.get(chat_id, "").strip() or "você"

def personalize(raw: str, chat_id: int, hora: str) -> str:
    return raw.replace("{nome}", name_of(chat_id)).replace("{hora}", hora)
...
def cta_keyboard_from_text(texto: str) -> InlineKeyboardMarkup:
    ...
    if not rows:
        rows = [[InlineKeyboardButton("⚡ Sessões do Dia", callback_data="sessoes")]]
...
# ============== MENSAGENS (90/ocasião + 30 boa-noite) ==============
...
# ============== BROADCASTS & TESTES ==============
HORAS = {"m": "10:00", "t": "15:00", "n": "20:00"}  # manhã 10h
...
async def post_m(c): await _broadcast(c, POS_M, "post_m", HORAS["m"])
async def post_t(c): await _broadcast(c, POS_T, "post_t", HORAS["t"])
async def post_n(c): await _broadcast(c, POS_N, "post_n", HORAS["n"])
...
def schedule_jobs(app: Application):
    jq = app.job_queue
    # MANHÃ 10:00
    jq.run_daily(pre_m, time=jitter(br_time(9, 50)))
    jq.run_daily(lambda c: during_burst(c, "m"), time=br_time(10, 0))
    jq.run_daily(post_m, time=jitter(br_time(10, 40)))
    # TARDE 15:00
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        return
    chat_id = update.effective_chat.id
    nome = USERS.get(chat_id)
    if nome:  # já fez onboarding
        await update.message.reply_text(
            f"Você já tá liberado, {nome}! 👇 Atalhos rápidos pra começar",
...
async def got_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Se já tem nome salvo → tenta captar intenção; senão mostra atalhos (fallback)
    if chat_id in USERS and USERS[chat_id]:
        intent = _match_intent(texto)
        if intent:
            await handle_intent(update, context)
            return
        await update.message.reply_text(
            "Se quiser, já pode começar por aqui 👇",
            reply_markup=fixed_shortcuts_keyboard()
        )
        return

    # Onboarding: salvar nome, enviar PDF e atalhos
    if not texto:
        return
    USERS[chat_id] = texto
    ...
    await update.message.reply_text(f"Shooow, {texto}! Parabéns por fazer parte do nosso time!\n\nAqui está seu bônus 👇")
...
async def test_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    SUBSCRIBERS.add(chat_id)
    save_state()
    await update.message.reply_text("✅ Teste iniciado (pré, durante, pós e boa-noite).")
